...
describe('TextInput', () => {
  it('renders default CSS class, custom CSS class, invalid and warning classes and size class as stipulated', async () => {
    ...
    await render(<CarbonRoot><TextInput class={customClass} invalid warn /></CarbonRoot>);
  ...
  it('removes non-standard properties from rendered input element if supplied', async () => {
    const {screen, render} = await createDOM();
    const nonStdProps = ['renderSize', 'labelText', 'hideLabel', 'helperText', 'inline', 'invalid', 'invalidText', 
    'warn', 'warnText', 'enableCounter', 'maxCount'];

    await render(<CarbonRoot><TextInput type="text" renderSize="sm" labelText="Label" hideLabel helperText='Helper text' inline invalid 
      invalidText='Invalid text' readOnly warn warnText='Warning text' enableCounter maxCount={10} /></CarbonRoot>);
