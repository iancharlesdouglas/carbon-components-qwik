...
Shows a list of options from which to select.  A popup listbox appears when the user clicks or uses the keyboard to open it.
...
If you define items as objects, each object must have a `label` property that contains the item's name, or you should define a render function to render its name or to render the component (see below).
E.g.
```
